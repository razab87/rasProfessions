...
- 1.5 (current): added build 42 multiplayer support; fixed clothes for homeless profession not washable although they can be dirty on start; fixed slow-reader trait is selectable by professions with the fast-reader trait (is not selectable anymore); in multiplayer, disabled the custom spawn points introduced by the mod (this is to avoid conflicts with the vanilla claim-safehouse functionality)
