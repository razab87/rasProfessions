...
- 1.4.5: only applies to B42: fixed profession icons sometimes not shown in-game after loading a game; fixed sometimes not possible to change to a modded professions ingame when playing in debug mode

- 1.4.6: updated for game version 42.13 single player; multiplayer not yet available

- 1.5: added build 42 multiplayer support; fixed homeless clothes not washable although they can be dirty on start; fixed slow-reader-trait is selectable by professions with the fast-reader-trait; in multiplayer, disabled the custom spawn points introduced by the mod (this is to avoid conflicts with the vanilla claim-safe-house functionality)

- 1.5.1 (current): adjusted the translations to the game's new json translation format


------------------------- TECHNICAL/FOR MODDERS/MISC -------------------------


Some of the professions added by this mod can also be found in other mods like "Dub's Profession Mod" or "Bef's Professions and Traits". (Have a look at them in the steam workshop!) However, the professions in this mod differ quite a lot in features like balancing, clothes, equipment etc.
...
The mod also changes the display name of a few clothing items for better interface behavior as well as some body locations. See this mod's file lua/shared/RasProfessionsItems.lua for more info.
