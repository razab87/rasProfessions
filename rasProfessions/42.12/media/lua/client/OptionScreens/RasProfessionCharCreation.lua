...
local function isShortSighted()
       ...
       if CharacterCreationProfession.instance.listboxTraitSelected and CharacterCreationProfession.instance.listboxTraitSelected.items then 
             ...
             for _,v in pairs(traits) do
                   if v and v.item:getType() == "ShortSighted" then
                       return true
                   end
             end
       end
...
local function manageClothing(self, desc, profession, gender, resetWornItems)
    ...
    if profession and not resetWornItems then -- only run this when in the           
        if rasDefinitions[profession] then
            ...
            if rasDefinitions[profession]["ShirtDecals"] then -- apply correct shirt decals
                local shirt = desc:getWornItem("Tshirt")
                if shirt then
    ...
    if dressUpDone then
        ...
        if desc:getWornItem("Eyes") then
            desc:setWornItem("LeftEye", nil)
        end
    end
end
...
local function manageHair(desc, profession, gender)
    ...
    if profession == "rasStudent" or profession == "rasGangMember" then
        ...
        if profession == "rasStudent" and gender == "Male" then -- students don't get full beards, goatee or mustache
    ...
    elseif profession == "rasRetired" then -- retired gets only grey hair
...
function CharacterCreationMain:initClothing(...)
    ...
    if self.clothingPanel then
        local desc = MainScreen.instance.desc
        local profession = desc:getProfession()
        if isShortSighted() and rasDefinitions[profession] and rasDefinitions[profession]["Prescription"] then
            addPrescGlasses(self, profession, desc)
        end
    end
end
...
local function updateDecalCombo(self, bodyLocation, item)
    ...
    if clothingItem and clothingItem:getDecalGroup() then
        ...
        local desc = MainScreen.instance.desc       
        local profession = desc:getProfession()
        if rasDefinitions[profession] and rasDefinitions[profession]["ShirtDecals"] and rasDefinitions[profession]["ShirtDecals"][itemType] then -- in case we have a custom list of decals
            local decalTable = rasDefinitions[profession]["ShirtDecals"][itemType]
...
function CharacterCreationMain:onClothingComboSelected(combo, bodyLocation, ...)
    ...
    if bodyLocation == "Tshirt" then -- we have decals definitions only for Tshirts 
        local desc = MainScreen.instance.desc       
        local profession = desc:getProfession() 
        if rasDefinitions[profession] and rasDefinitions[profession]["ShirtDecals"] then
            ...
            if _self.lastIndex ~= _index then
                ...
                if bodyLocation == "Tshirt" then         
                    ...
                    if rasDefinitions[profession] and rasDefinitions[profession]["ShirtDecals"] then                   
                        ...
                        if item then
                            local itemType = item:getFullType()                         
                            if rasDefinitions[profession]["ShirtDecals"][itemType] then
                                local decals = rasDefinitions[profession]["ShirtDecals"][itemType]
                                local decal = decals[ZombRand(0, #decals) + 1]
                                item:getVisual():setDecal(decal)
                                self:updateSelectedClothingCombo()
