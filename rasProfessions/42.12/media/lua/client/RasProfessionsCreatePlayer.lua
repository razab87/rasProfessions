...
local function makeInventory(player)
         ...
         -- remove vanilla starting gun
         if SandboxVars.RasProfessions.RemoveVanillaGun then
            playerInv:RemoveAll("Base.Pistol")
            playerInv:RemoveAll("Base.9mmClip")
            playerInv:RemoveAll("Base.Bullets9mmBox")
         end
...
local function manageTraits(player)         
       ...
       for _,traitPair in pairs(rasMainData.traitsToSwap) do 
              if player:HasTrait(traitPair.swap) then                                      
                   if traitPair.swap ~= "rasOutOfShapeProf" then -- outOfShape trait requires special treatment in FirstTicks.lua                                            
...
local function createPlayer(player, square)
         ...
         rasMainData.NewGameStarted = true
