    ...
    if square then        
        ...
        if keyring then 
            ...
            if building then
                ...
                if buildingDef:isResidential() and not buildingDef:isShop() and player:getDescriptor():getCharacterProfession() ~= Regs.NewProfs.rasHomeless then
...
local function removeBasementZombies(tick)
    ...
    if square and square:getZ() < 0 and not discoverTick then 
        ...
        if xDiff < 22 and yDiff < 22 then -- make sure the basement we discovered is not far away from starter house
            discoverTick = tick
        end
    end
    ...
    if discoverTick and tick >= discoverTick + 2 then -- wait two ticks before removing
        ...
        if cell then
            ...
            for i=zombies:size(),1,-1 do -- note: list gets smaller when removing zeds, therefore start with i=size
                local zombie = zombies:get(i-1)
                local sq = zombie:getSquare()
                if sq and sq:getZ() < 0 then --only remove basement zeds
                    zombie:removeFromWorld()
                    zombie:removeFromSquare()
...
local function spawn(player)
    ...
    local profession = player:getDescriptor():getCharacterProfession():toString()
    local mod, profID
    if profession and profession:find(":") then
        mod, profID = profession:match("^(.-:)(.*)$")
    end

    local city = rasSharedData.StartingCity
    if (mod == "base:" or mod  == "rasprofs:") and city and rasSharedData["Spawns"][city] and rasSharedData["Spawns"][city][profID] then
        local spawns = rasSharedData["Spawns"][city][profID]
            ...
            if point["worldX"] then
                ...
                if point["worldY"] then
                    y = (point["worldY"]*300) + point["posY"]
                     ...
                     DebugLog.log("rasProfessions_WARN: Spawnpoint for " .. profID .. " in " .. city .. " not correctly defined!")
                end   
            else
               x = point["posX"]  
               y = point["posY"]
        ...
        if x and y then
            ...
            if building then
                ...
                building:setAllExplored(true) -- this might prevent a house from becoming randomized (burnt, barricaded...) but not 100% sure
...
local function mainFunction(player, square)
    ...
    if not isClient() then -- custom spawn points only in SP 
