...
function CharacterCreationProfession:populateProfessionList(list, ...)
    ...
    list:clear() -- we have to clear the list before we start; vanilla game is designed for calling the function only once and will double elements if we call it manually several times without clearing

    vanilla_populateProfessionList(self, list, ...) --execute vanilla code

    local listBackUp = {}
    for _,v in pairs(list.items) do -- backup old list
        table.insert(listBackUp, v)    
    end
    ...
    for _,v in pairs(listBackUp) do
        if not excludedProfessions[v.item:getType()] then -- rewrite list but exclude professions we want to omit
            list:addItem(v.text, v.item, v.tooltip)    
        end
    end
