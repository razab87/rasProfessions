-- patch the BuildAction so that walls and doors constructed by architect get more durability; this build action is only relevant in singleplayer; for multiplayer, we patch Actions.build in shared folder instead
--
--
-- by razab
...
function ISBuildAction:perform(...)
    ...
    if not isClient() and self.item and self.item.craftRecipe and self.square then -- only in SP
        ...
        if self.character:getDescriptor():getCharacterProfession() == Regs.NewProfs.rasArchitect and (isWall[recipeName] or isDoor[recipeName]) then
            ...
            for i=0, oldObjects:size() - 1 do
                ...
                if object:getName() == recipeName then
                    isOldObject[object] = true -- store old wall
                end
            end 
        end
    end
    ...
    if increaseHP then
        ...
        for i=0, newObjects:size() - 1 do
            ...
            if object:getName() == recipeName and not isOldObject[object] then -- if it is the newly constructed object 
